...
router.get('/', requirePerfil('ADMIN', 'EMPRESA_ADMIN'), async (req, res) => {
  ...
  let sql = `
    SELECT u.id_usuario, u.nome_usuario, u.email, u.cargo, e.nome_fantasia as empresa, u.perfis
    FROM usuario u
...
router.post('/', requirePerfil('ADMIN', 'EMPRESA_ADMIN'), async (req, res) => {
  ...
  try {
    const { rows: uRows } = await pool.query(
      `INSERT INTO usuario (nome_usuario, email, senha_hash, id_empresa, cargo, perfis)
       VALUES ($1, $2, crypt($3, gen_salt('bf')), $4, $5, $6) RETURNING id_usuario`,
      [nome_usuario, email, senha, id_empresa, cargo || '', [perfil_codigo]]
    );
    const userId = uRows[0].id_usuario;

    if (perfil_codigo === 'TECNICO') {
      await pool.query('INSERT INTO tecnico (id_usuario, especialidade) VALUES ($1, $2)', [userId, 'Geral']);
    }
